...
const weighted_round_robin string = `{"loadBalancingConfig": [{"weighted_round_robin": {}}]}`

// weighted_round_robin 是grpc中的负载均衡策略，round_robin是轮询，weighted_round_robin是加权轮询
func (s *BalancerTestSuite) TestClientWRR() {
	...
	cc, err := grpc.NewClient("etcd:///service/user",
		...
		//}
		//`),
		grpc.WithDefaultServiceConfig(weighted_round_robin),
		grpc.WithTransportCredentials(insecure.NewCredentials()))
	require.NoError(t, err)
	client := NewUserServiceClient(cc)
	for i := 0; i < 10; i++ {
...
// round_boin 是grpc中的负载均衡策略，round_robin是轮询，weighted_round_robin是加权轮询
const round_boin string = `{"loadBalancingConfig": [{"round_robin": {}}]}`
...
func (s *BalancerTestSuite) TestClient() {
	...
	cc, err := grpc.NewClient("etcd:///service/user",
		...
		//}
		//`),
		grpc.WithDefaultServiceConfig(round_boin),
		grpc.WithTransportCredentials(insecure.NewCredentials()))
	require.NoError(t, err)
	client := NewUserServiceClient(cc)
	for i := 0; i < 10; i++ {
...
func (s *BalancerTestSuite) TestServer() {
	go func() {
		s.startServer(":8090", 10, &Service{
			Name: ":8090",
		})
	}()
	go func() {
		s.startServer(":8091", 20, &Service{
			Name: ":8091",
		})
	}()
